...
def model_inference(model, data_loader, device, params = Params()):
    ...
    for i, images in enumerate(tqdm(data_loader, desc='Hidden Set Inference')):
        ...
        images = images[:, -1, :, :, :].to(device) # only use the 11th frame
        ...
        if mask_pred_argmax.shape[1] != 160 or mask_pred_argmax.shape[2] != 240:
            ...
            mask_pred_argmax = v2.Resize((160, 240), interpolation=InterpolationMode.NEAREST)(mask_pred_argmax)

        if i % 1000 == 0: # visualize every 1000th mask
            visualize_predicted_example(images[0].detach().cpu(), None, mask_pred_argmax.detach().cpu(), f"Predicted Mask for Hidden Set Video {i} based on Frame {11} only", os.path.join(params.hidden_set_mask_dir, f"hidden_{i}.png"), params)
